...
function packagesWithPinnedVersions(dependencyTree) {
  ...
  for (const node of dependencyTree.inventory.values()) {
    ...
    for (const dependencyName in node.packageInfo.dependencies || {}) {
      if (node.packageInfo.dependencies[dependencyName].substring(0, 1) === '~') {
  ...
  return {
    ...
    message: `There are currently ${(new Set(packagedWithPinned.map((action) => action.meta.name))).size
...
function packagesWithExtraArtifacts(dependencyTree) {
  ...
  for (const node of dependencyTree.inventory.values()) {
    ...
    if (fs.existsSync(path.resolve(node.path, "docs"))) {
      ...
      extraArtifacts.push({
        message: `"${node.name}" (${breadcrumb}) has a "docs" folder which is not necessary for production usage ${humanFileSize(size)}.`,
        meta: {
          breadcrumb,
          name: node.name,
          directory: node.path,
    ...
    if (fs.existsSync(path.resolve(node.path, "tests"))) {
      ...
      extraArtifacts.push({
        message: `"${node.name}" (${breadcrumb}) has a "tests" folder which is not necessary for production usage ${humanFileSize(size)}.`,
        meta: {
          breadcrumb,
          name: node.name,
          directory: node.path,
  ...
  return {
    ...
    message: `There are currently ${(new Set(extraArtifacts.map((action) => action.meta.name))).size
...
function topLevelDepsFreshness(dependencyTree, latestPackages) {
  ...
  for (const dependency in dependencies) {
    try {
      ...
      const topLevelPackage = [...dependencyTree.inventory.values()].find((dependency) => dependency.location === topLevelPackagePath);
  ...
  return {
    ...
    message: `Out of the total ${totalDeps} explicit dependencies defined in the package.json; ${outOfDate.major.length
...
function notBeingAbsorbedByTopLevel(dependencyTree) {
  ...
  for (const node of dependencyTree.inventory.values()) {
    ...
    if (topLevelPackage && topLevelPackage.version !== node.version) {
      ...
      notAbsorbed.push({
        message: `"${node.name
          }" (${breadcrumb}) not absorbed because top level dep is "${topLevelPackage.version
          }" and this is "${node.version
  ...
  return {
    ...
    message: `There are currently ${notAbsorbed.length
...
function nestedDependencyFreshness(dependencyTree, latestPackages) {
  ...
  return {
    ...
    message: `Out of the total ${totalDeps} sub packages currently installed; ${outOfDate.major.length
      } major versions out of date (${(
        (outOfDate.major.length / totalDeps) *
        100
      ).toFixed(2)}%), ${outOfDate.minor.length} minor versions out of date (${(
...
async function generateReport(dependencyTree) {
  ...
  return {
    ...
    dependencies: [...dependencyTree.inventory.entries()].map((arr) => {
      ...
      dependencyInfo.packageInfo = JSON.parse(fs.readFileSync(path.resolve(dependencyInfo.path, 'package.json'), 'utf8'))
