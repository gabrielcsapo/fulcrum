...
function topLevelDepsFreshness(dependencyTree, latestPackages) {
  ...
  for (const dependency in dependencies) {
    const topLevelPackage =
      dependencyTree.meta.data.packages["node_modules/" + dependency];

    const diff = semverDiff(
      topLevelPackage.version,
      latestPackages[dependency]
    );
  ...
  return {
    ...
    actions: [
      ...outOfDate.major.map(({ name, directory, version, path }) => {
        return {
      ...
      ...outOfDate.minor.map(({ name, directory, version, path }) => {
        return {
      ...
      ...outOfDate.patch.map(({ name, directory, version, path }) => {
        return {
...
function notBeingAbsorbedByTopLevel(dependencyTree) {
  ...
  for (const node of dependencyTree.inventory.values()) {
    ...
    if (topLevelPackage && topLevelPackage.version !== node.version) {
      ...
      notAbsorbed.push({
        message: `"${node.name}" not absorbed because top level dep is "${
          topLevelPackage.version
        }" and this is "${
          node.version
        }". This takes up an additional ${humanFileSize(size)}`,
        meta: {
          path: getBreadcrumb(node),
          name: node.name,
          directory: node.path,
          size,
        },
...
async function generateReport(dependencyTree) {
  ...
  // todo: use this to write out the latestPackage during dev mode. This should be better.
  // fs.writeFileSync("temp.json", JSON.stringify(latestPackages));
  const latestPackages = require("../temp.json");
  ...
  return {
    ...
    dependencies: [...dependencyTree.inventory.entries()],
    suggestions: [
